...
uint8_t PS2_SendByte(uint8_t cmd) {
	...
	for (buffer = 0x01; buffer < 0x0100; buffer <<= 1) {
		if (buffer & cmd) {
			HAL_GPIO_WritePin(MOSI2_GPIO_Port, MOSI2_Pin, GPIO_PIN_SET);
		} else {
			HAL_GPIO_WritePin(MOSI2_GPIO_Port, MOSI2_Pin, GPIO_PIN_RESET);
		}
		HAL_GPIO_WritePin(SCK2_GPIO_Port, SCK2_Pin, GPIO_PIN_SET);
		delay_us(8);
		HAL_GPIO_WritePin(SCK2_GPIO_Port, SCK2_Pin, GPIO_PIN_RESET);
		delay_us(8);
		HAL_GPIO_WritePin(SCK2_GPIO_Port, SCK2_Pin, GPIO_PIN_SET);
		if (HAL_GPIO_ReadPin(MISO2_GPIO_Port, MISO2_Pin)) {
			data |= buffer;
		}
	}
	delay_us(8);
